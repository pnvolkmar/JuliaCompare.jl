...
push!(dimension_filters, :AreaTOM => ["AB"])
push!(dimension_filters, :ECC => ["SingleFamilyDetached","SingleFamilyAttached",
"MultiFamily","OtherResidential","Passenger"])
push!(dimension_filters, :Year => "2050")

################################################################################
# Analysis of variables ########################################################
################################################################################
PExp = J.diff_fast("PExp", loc1, loc2; dimension_filters, sec)
@rsubset! PExp abs(:Diff) > 1e-5
PExp_g = @by PExp :ECC :Spruce = sum(:Spruce) :Tanoak = sum(:Tanoak) :Diff = sum(:Diff)
J.add_pdiff!(PExp_g)

EPermitHHe = J.diff_fast("EPermitHHe", loc1, loc2; dimension_filters, sec)
J.add_pdiff!(EPermitHHe)
################################################################################
ind = copy(dimension_filters)
push!(ind, :ECCintoTOM => "OilGasNonConventional")
push!(ind, :Poll => ["N2O", "CO2", "CH4"])
################################################################################

EC = M.ReadDisk(loc2.HDF5_path, "IInput/ECKey")
push!(ind, :ECC => EC)
@rsubset! EPermitE abs(:PDiff) > 10

EPermitE = J.diff_fast("EPermitE", loc1, loc2; dimension_filters = ind, sec)
J.add_pdiff!(EPermitE)

MapECCintoTOM = J.diff_fast("MapECCintoTOM", loc1, loc2; dimension_filters=ind, sec)
@rsubset MapECCintoTOM :Spruce != 0 :ECCintoTOM == "OilGasNonConventional"

MapECCintoTOM_j = M.ReadDisk(loc2.HDF5_path,"KInput/MapECCintoTOM") # [ECC,ECCintoTOM] Map between ECCintoTOM and ECC
findall(MapECCintoTOM_j .!= 0)

SplitECCintoTOM = J.diff_fast("SplitECCintoTOM", loc1, loc2; dimension_filters=ind, sec)
@rsubset! SplitECCintoTOM :Spruce != 0 :ECCintoTOM == "OilGasNonConventional"
push!(ind, :ECC => unique(SplitECCintoTOM.ECC))

PExp = J.diff_fast("PExp", loc1, loc2; dimension_filters=ind, sec)
@rsubset! PExp abs(:Diff) > 1e-5
PExp_g = @by PExp :ECC :Spruce = sum(:Spruce) :Tanoak = sum(:Tanoak) :Diff = sum(:Diff)
J.add_pdiff!(PExp_g)

#    PExp[ecc,poll,area] = PExpGross[ecc,poll,area]-OffRevenue[ecc,poll,area]
PExpGross = J.diff_fast("PExpGross", loc1, loc2; dimension_filters=ind, sec)
ETAvPr = J.diff_fast("ETAvPr", loc1, loc2; dimension_filters=ind, sec)
@rsubset ETAvPr abs(:Diff) > 1e-5
market = copy(ind)
pop!(market, :Year)
push!(market, :Market => "Market1")
ETAvPr = J.diff_fast("ETAvPr", loc1, loc2; dimension_filters=market, sec)
@rsubset ETAvPr abs(:Diff) > 1e-5
ETAPr = J.diff_fast("ETAPr", loc1, loc2; dimension_filters=market, sec)
@rsubset ETAPr :Year > 2016 :Year < 2019
push!(market, :EC => market[:ECC])
ECoverage = J.diff_fast("ECoverage", loc1, loc2; dimension_filters=market, sec='I')
@rsubset ECoverage :Year == 2050 :EC == "PrimaryOilSands"
Enforce = J.diff_fast("Enforce", loc1, loc2; dimension_filters=market, sec)
YrFinal = J.diff_fast("YrFinal", loc1, loc2; dimension_filters=market, sec)
ECCMarket = J.diff_fast("ECCMarket", loc1, loc2; dimension_filters=market, sec)
@rsubset ECCMarket :Spruce > 0

on = Dict{Symbol,Any}()
push!(on, :Area => "ON")
push!(on, :Poll => "CO2")
push!(on, :Year => ["2021","2022"])
push!(on, :ECC => ["SingleFamilyDetached"])
push!(on, :EC => ["SingleFamilyDetached"])

PExp = J.diff_fast("PExp", loc1, loc2; dimension_filters=on, sec='R') # bad
PBuy = J.diff_fast("PBuy", loc1, loc2; dimension_filters=on, sec='R') # good
PSell = J.diff_fast("PSell", loc1, loc2; dimension_filters=on, sec='R') # good
PExpGross = J.diff_fast("PExpGross", loc1, loc2; dimension_filters=on, sec='R') # bad
PGratis = J.diff_fast("PGratis", loc1, loc2; dimension_filters=on, sec='R') # good
@rsubset! PGratis :Diff != 0
CapTrade = J.diff_fast("CapTrade", loc1, loc2; dimension_filters=on, sec='R') # bad
@rsubset! CapTrade :Spruce != 0
markets_captrade = unique(CapTrade.Market)
ECCMarket = J.diff_fast("ECCMarket", loc1, loc2; dimension_filters=on, sec='R') # good
@rsubset! ECCMarket :Spruce != 0
markets_eccmarket = unique(ECCMarket.Market)
AreaMarket = J.diff_fast("AreaMarket", loc1, loc2; dimension_filters=on, sec='R') # good
@rsubset! AreaMarket :Spruce != 0
markets_areamarket = unique(AreaMarket.Market)
push!(on, :Market => intersect(markets_captrade, markets_eccmarket, markets_areamarket))

ECoverage = J.diff_fast("ECoverage", loc1, loc2; dimension_filters=on, sec='R') # good

TBuy = J.diff_fast("TBuy", loc1, loc2; dimension_filters=on, sec='R') # bad?
TSell = J.diff_fast("TSell", loc1, loc2; dimension_filters=on, sec='R') # bad
TGratis = J.diff_fast("TGratis", loc1, loc2; dimension_filters=on, sec='R') # bad!
# TSell(Market)=xPAuction(Market)+
#   sum(ECC,Poll,Area)(PSell(ECC,Poll,Area))+FSell(Market)+xXISell(Market)+
#   OffTotal(Market)
J.diff_fast("xPAuction", loc1, loc2; dimension_filters=on, sec='R') # good
J.diff_fast("FSell", loc1, loc2; dimension_filters=on, sec='R') # good
J.diff_fast("xXISell", loc1, loc2; dimension_filters=on, sec='R') # good
J.diff_fast("OffTotal", loc1, loc2; dimension_filters=on, sec='R') # good
# which tells my PSell is off for some ECCs in market 205
pop!(on, :ECC)
J.diff_fast("PSell", loc1, loc2; dimension_filters=on, sec='R') # good but meaningless
@rsubset! PSell :Diff != 0
J.diff_fast("MktPol", loc1, loc2; dimension_filters=on, sec='R') # bad?
J.diff_fast("PInvChange", loc1, loc2; dimension_filters=on, sec='R') # 0s
PolCov = J.diff_fast("PolCov", loc1, loc2; dimension_filters=on, sec='R') # issue
@rsubset! PolCov :Diff != 0
PolTot = J.diff_fast("PolTot", loc1, loc2; dimension_filters=on, sec='R') # issue
@rsubset! PolTot :Diff != 0
EuFPol = J.diff_fast("EuFPol", loc1, loc2; dimension_filters=on, sec='R') # issue
@rsubset! EuFPol abs(:Diff) >= 1
PCovMap = J.diff_fast("PCovMap", loc1, loc2; dimension_filters=on, sec='R') # issue
@rsubset! PCovMap abs(:Diff) != 0
OUDmdFr = J.diff_fast("OUDmdFr", loc1, loc2; dimension_filters=on, sec='R') # issue
@rsubset! OUDmdFr abs(:Diff) != 0

# julia> @rsubset! PCovMap abs(:Diff) != 0
# 8×8 DataFrame
#  Row │ FuelEP         ECC                   PCov        Area    Year   Spruce   Tanoak   Diff    
#      │ String         String                String      String  Int64  Float64  Float64  Float64
# ─────┼───────────────────────────────────────────────────────────────────────────────────────────
#    1 │ NaturalGas     SingleFamilyDetached  Energy      ON       2021      0.0      1.0     -1.0
#    2 │ NaturalGasRaw  SingleFamilyDetached  Energy      ON       2021      1.0      0.0      1.0
#    3 │ NaturalGasRaw  SingleFamilyDetached  Oil         ON       2021      0.0      1.0     -1.0
#    4 │ NaturalGas     SingleFamilyDetached  NaturalGas  ON       2021      1.0      0.0      1.0
#    5 │ NaturalGas     SingleFamilyDetached  Energy      ON       2022      0.0      1.0     -1.0
#    6 │ NaturalGasRaw  SingleFamilyDetached  Energy      ON       2022      1.0      0.0      1.0
#    7 │ NaturalGasRaw  SingleFamilyDetached  Oil         ON       2022      0.0      1.0     -1.0
#    8 │ NaturalGas     SingleFamilyDetached  NaturalGas  ON       2022      1.0      0.0      1.0

ETAPr = J.diff_fast("ETAPr", loc1, loc2; dimension_filters=on, sec='R') # good
ETADAP = J.diff_fast("ETADAP", loc1, loc2; dimension_filters=on, sec='R') # good
Enforce = J.diff_fast("Enforce", loc1, loc2; dimension_filters=on, sec='R') # bad
